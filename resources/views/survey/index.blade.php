...
@endif
    ...
    <style type="text/css">
                    ...
                    <a href="/survey/create" class="btn btn-success"><i class="fa fa-plus"></i>&nbsp Input Data Survey Baru</a>
                    <br/>
                    <br/>
                    ...
                    <table class="table table-striped" id="table-data">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                @php
                                    $foto = App\Surveyimage::where('survey_unique_id',$p->unique_id)->get();
                                  @endphp
                                  ...
                                  @endforeach
                                <!-- <a href="{{ asset('gambar/'. $p->foto ) }}" target="_blank" rel="noopener noreferrer"><img src="{{ asset('gambar/'. $p->foto ) }}" height="10%" width="10%" alt=""></a>
                                <a href="{{ asset('gambar/'. $p->foto2 ) }}" target="_blank" rel="noopener noreferrer"><img src="{{ asset('gambar/'. $p->foto2 ) }}" height="10%" width="10%" alt=""></a>
                                <a href="{{ asset('gambar/'. $p->foto3 ) }}" target="_blank" rel="noopener noreferrer"><img src="{{ asset('gambar/'. $p->foto3 ) }}" height="10%" width="10%" alt=""></a>
                                <a href="{{ asset('gambar/'. $p->foto4 ) }}" target="_blank" rel="noopener noreferrer"><img src="{{ asset('gambar/'. $p->foto4 ) }}" height="10%" width="10%" alt=""></a>
                                <a href="{{ asset('gambar/'. $p->foto5 ) }}" target="_blank" rel="noopener noreferrer"><img src="{{ asset('gambar/'. $p->foto5 ) }}" height="10%" width="10%" alt=""></a></td>
                                --> <td>{{ $p->nama_pensurvey }}</td>
                                <td><label class="label label-success">{{ ($p->status == 0) ? 'Lolos' : 'Process' }}</label></td>
                                </td>
                                <td>
                                    @if($p->status == 1)
                                    <a href="/survey/status/{{ $p->nik }}" class="btn btn-sm btn-warning" onClick="return confirm('Yakin ingin meluluskan data ini')">ACC</a>
                                    @else
                                    <a href="#" class="btn btn-sm btn-primary" disabled>Lolos</a>
                                    @endif
</td>
<!-- <td>
                                    <a href="/survey/show/{{ $p->nik }}" class="btn btn-warning"><i class="fa fa-eye"></i></a>
                                </td> -->
<!-- <td>
    ...
    <form method="post" action="/survey/destroy{{ $p->nik }}"> {{ csrf_field() }}
        ...
        <td>
          <a href="/pemohon/destroy/{{ $p->nik }}" class="btn btn-danger"><i class="fa fa-trash"></i></a>
                    ...
                    <br/>
                    Halaman : {{ $survey->currentPage() }}<br/>
                    Jumlah Data : {{ $survey->total() }}<br/>
                    Data Per Halaman : {{ $survey->perPage() }}<br/>
